...
## 핵심 결과 (Key Findings)
- MLP 기반 시계열 예측으로 분야별 ODA의 수혜 효과를 정량화 가능
- XGBoost와 SHAP 기반 중요도 분석 결과, 분야별 지원 내역과 특정 개발 지표 간 시차 기반 상관관계 존재
- ODA Impact Simulator는 특정 국가에 대해 입력된 목적별 지원 비중을 바탕으로 개발 지표 변화 예측 가능
- UNDP에서는:
  - 사전 정책 시뮬레이션 도구로 활용 가능
  - 특정 국가/분야에 대한 ODA 배분 전략 수립 시 참고 자료 제공 가능
<br>
