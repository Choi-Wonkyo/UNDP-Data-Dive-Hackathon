...
def load_data():
    ...
    observed = pd.read_csv(r"C:\Users\user\Desktop\CYClone_UNDP Project Streamlit\data\df_ready.csv")
    return oda, coords, observed

oda_df, coords_df, observed_outcomes_df = load_data()

country_list = sorted(coords_df["country"].unique())
...
# ====== 지도 데이터 처리 ======
oda_for_map = (
    oda_df[oda_df["RecipientName"].isin(country_list)]
    .dropna()
    .groupby(["Year", "RecipientName"])["USD_Disbursement"]
    .sum()
    .reset_index()
)
...
recipient_merged = recipient_merged.dropna(subset=["latitude", "longitude", "USD_Disbursement"])
...
numerical_cols = [
    ...
    if (not col.startswith("country_") and "_증가율" not in col and "_감소율" not in col)
]
...
if st.session_state["show_map"]:
    ...
    st.write("Simulate the expected outcome changes based on ODA variation.")
    ...
    with col2:
        ...
        if input_df.isnull().sum().sum() > 0:
            st.error("❌ 누락된 입력이 있어 예측할 수 없습니다.")
            st.stop()
        ...
        # --- 실제 관측값 덮어쓰기 (모든 multiplier가 1.0일 때) ---
        all_ones = all([
            primary_enrollment_ratee == 1.0,
            infant_mortality == 1.0,
            neonatal_mortality_rate == 1.0,
            life_expectancy == 1.0,
            gni_per_capita == 1.0,
            value_added_services == 1.0,
            value_added_manufacturing == 1.0,
            crop_production_index == 1.0,
            livestock_production_index == 1.0,
            gdp_per_capita == 1.0,
            renewable_energy_usage == 1.0,
            air_pollution_index == 1.0,
            co2_emissions_per_capita == 1.0,
        ])
        if all_ones:
            row = observed_outcomes_df[
                (observed_outcomes_df["country"] == selected_country) &
                (observed_outcomes_df["year"] == selected_year)
            ]
            if not row.empty:
                output_result = np.array([
                    row["교육_초등학교 순취학률_증가율"].values[0],
                    row["보건_영아 사망률_감소율"].values[0],
                    row["보건_기대 수명_증가율"].values[0],
                    row["빈곤 및 사회복지_1인당 GNI_증가율"].values[0],
                    row["경제_1인당 GDP_증가율"].values[0],
                    row["환경/기후_재생 에너지 사용률_증가율"].values[0],
                    row["환경/기후_1인당 co2 배출량_감소율"].values[0],
                ])
    ...
    with col1:
        ...
        delta_signs = ['▲' if d > 0 else '▼' if d < 0 else '-' for d in delta_values]
        delta_texts = [
            f"{val:.2f}% ({sign} {abs(delta):.2f}%)"
            for val, sign, delta in zip(output_result, delta_signs, delta_values)
        ]
        ...
        fig_bar.update_layout(
            title='Predicted Outcomes (User Input)',
            xaxis=dict(title='Predicted Change (%)', range=[-30, 30], zeroline=True, zerolinecolor='black'),
        ...
        radar_dummy = list(dummy_result) + [dummy_result[0]]
        radar_user = list(output_result) + [output_result[0]]
        delta_radar = output_result - dummy_result
        delta_signs_radar = ['▲' if d > 0 else '▼' if d < 0 else '-' for d in delta_radar]
        delta_colors_radar = ['green' if d > 0 else 'red' if d < 0 else 'gray' for d in delta_radar]
        delta_texts_radar = [f"{sign} {abs(d):.2f}%" for sign, d in zip(delta_signs_radar, delta_radar)]
        ...
        with col_r2:
            ...
            label_r = [max(radar_user[:-1]) * 1.05] * len(radar_labels)
            fig_radar_user.add_trace(go.Scatterpolar(
                r=label_r,
                theta=radar_labels,
                mode='text',
                text=delta_texts_radar,
                textfont=dict(size=13, color=delta_colors_radar),
                hoverinfo='skip',
                showlegend=False
            ))
            fig_radar_user.update_layout(
                polar=dict(radialaxis=dict(visible=True, showticklabels=False)),
                title="User Customized ODA Impact"
            )
            st.plotly_chart(fig_radar_user, use_container_width=True)

        # Policy Insight
        if not np.allclose(output_result, dummy_result, atol=1e-4):
            ...
            st.markdown(
                ...
                ">
                    ...
                    해당 지표들은 ODA 조정에 민감하므로 전략적 우선순위로 고려할 수 있습니다.
                </div>
                """,
                unsafe_allow_html=True
            )
